...
// the same key pair, so a server can regenerate it on every start without
// persisting anything.
...
// ECHConfig. Meant to be persisted (see SaveECHKey) so it stays stable across
// restarts independently of any seed.
...
func validateECHPublicName(publicName string) error {
	if publicName == "" {
		return errors.New("empty ECH public name")
	}
	if len(publicName) > 255 {
		return errors.New("ECH public name too long (max 255 bytes)")
	}
	return nil
}
